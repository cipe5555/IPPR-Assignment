function glove_bounding_box = find_glove_contour(image)
    ...
    glove_bounding_box = imcrop(image, glove_bbox);

    % % Create mask using largest contour
    % mask = poly2mask(largest_contour(:,2), largest_contour(:,1), size(glove_binary,1), size(glove_binary,2));
    % 
    % % Crop ROI from original image using mask
    % glove_contour = bsxfun(@times, image, cast(mask, class(image)));
