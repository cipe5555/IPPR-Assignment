...
figure;imshow(finger_only_mask);
...
for k = 1:length(boundaries)
    ...
    if size(main_glove_contour, 1) - size(boundary, 1) > size(main_glove_contour, 1) - (size(main_glove_contour, 1)*0.9)
        ...
        if is_inside
            ...
            elseif ~isempty(finger_bbox)
                rectangle('Position',finger_bbox,'EdgeColor','b','LineWidth',2);
                % Get the position for the text label
                textPosition = [finger_bbox(1) + finger_bbox(3)/2, finger_bbox(2) + finger_bbox(4)/2];
                text(textPosition(1), textPosition(2), 'Missing Finger', 'Color', 'r', 'HorizontalAlignment', 'center', 'VerticalAlignment', 'middle');
            end
        end
    end
end
